...
1. Maintain manual metadata in:
   - `nomad_plugin_metadata.manual.yaml` (maintainer-owned focused template for manual/conditional fields)
...
- Reset report only:
  - remove `.metadata/*`
- Reset autogenerated artifacts (keep manual overrides):
  - remove `nomad_plugin_metadata.auto.yaml`, `nomad_plugin_metadata.yaml`, and `.metadata/*`
- Full reset (including manual curation):
  - also remove `nomad_plugin_metadata.manual.yaml`
